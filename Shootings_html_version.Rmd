...
plot_gender <- ggplot(shootings_per_gender, aes(y=shootings, x=reorder(gender, -shootings))) + 
...
plot_semi_single_bar <- ggplot(shootings_per_semi, aes(fill=semi_auto_used, y=shootings, x=1)) + 
...
# Display the single stacked bar
plot_semi_single_bar
...
# Create a single stacked bar
plot_multi_single_bar <- ggplot(shootings_per_multiple, aes(fill=multiple_weapons_used, y=shootings, x=1)) + 
    geom_bar( stat="identity") +
  coord_flip() +
  theme(
    axis.text.x = element_blank(),
...
# Create a bar chart using ggplot2 to show shootings by year and age bracket
 plot_year_age <- ggplot() + 
  geom_bar(aes(y=Num_Shootings, x=Year, fill=Age_Bracket), data = shootings_per_year_age, stat = "identity") +
  scale_fill_manual(values = c("#49a4aa", "#46b7e1", "#e14646", "#e27f46", "#e1ad46", "#666666", "#e0e1e2")) +
  labs(x = "Year",
       y = "Number of mass shootings") +
  theme(
