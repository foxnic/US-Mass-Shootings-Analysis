...
# Create a new data frame containing only the shootings from 2018 onwards
cleansed_data_2018_to_2019 <- data.frame(cleansed_data[cleansed_data$year > "2017",c("case", "international_date", "total_victims")])

# Create a new data frame that shows number of victims per date from 2018
victims_per_date_2018_2019 <- aggregate(cleansed_data_2018_to_2019$total_victims, by=list(cleansed_data_2018_to_2019$international_date),FUN = sum)

# Change the column heading names to "date" and "victims"
colnames(victims_per_date_2018_2019) <- c("date", "victims")

# Sort the data by date
victims_per_date_2018_2019 <- victims_per_date_2018_2019[order(as.Date(victims_per_date_2018_2019$date, format="%Y/%m/%d")),]

# Create a new data frame containing the rows in victims_per_date_2017_2019 plus rows with 0 victims for all the dates on which there were no mass shootings. Start by creating a list of all the dates between the 1st shooting and the latest.
earliest_shooting <- as.Date(min(victims_per_date_2018_2019$date))
most_recent_shooting <- as.Date(max(victims_per_date_2018_2019$date))
...
victims_per_date_2018_2019 <- right_join(victims_per_date_2018_2019, date_range, by = c("date"="date"))

# Change the column heading names to date, victims, temp_victims
colnames(victims_per_date_2018_2019) <- c("date", "victims", "temp_victims")

# If victims is NA then set it to zero
victims_per_date_2018_2019[is.na(victims_per_date_2018_2019$victims), c("victims")] <- 0

# Remove the temp_victims column
victims_per_date_2018_2019 <- victims_per_date_2018_2019[, c("date", "victims")]

```

```{r display_timeline, echo=FALSE, fig.height=2, fig.width=10}

# Create a single time line with bubbles representing victims by date for shootings in 2018 onwards

#set.seed(42) #only needed if I use geom_label_repel, which I can't get to work at the moment

timeline_chart <- ggplot(data = victims_per_date_2018_2019, aes(x = date, y = 0)) +
  #geom_hline(yintercept=0, linetype="solid", color = "#bcbaba", size=1)+
  ...
  geom_text(data = subset(victims_per_date_2018_2019, date == "2018-01-28" | date == "2019-02-15"), 
            aes(label = date), size = 4, color = "#666666", 
            hjust=-0.3, vjust=-0.3, angle=65)+
  geom_label(aes(x = as.Date("2018-07-30"), y = 0.095, label = "This is the 30th of July"), 
             hjust = 0, vjust = 0.5, lineheight = 0.8,
             colour = "#555555", fill = "white", label.size = NA, family="Helvetica", size = 6)+ 
  geom_segment(aes(x = as.Date("2018-07-30"), y = 0.005, xend = as.Date("2018-08-15"), yend = 0.08), 
               colour = "#555555", size=0.5)+
  geom_label(aes(x = as.Date("2018-05-01"), y = 0.095, label = "This is the 1st of May"), 
             hjust = 0, vjust = 0.5, lineheight = 0.8,
             colour = "#555555", fill = "white", label.size = NA, family="Helvetica", size = 6)+ 
  geom_segment(aes(x = as.Date("2018-05-01"), y = 0.005, xend = as.Date("2018-05-16"), yend = 0.08), 
               colour = "#555555", size=0.5)
