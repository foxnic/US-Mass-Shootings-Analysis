...
treemap_state_shootings <- ggplot(shootings_per_state, aes(area = shootings, label = state)) +
  geom_treemap(aes(alpha = shootings)) +
...
**19** (17%) of the mass shootings were in California.
...
plot_race <- ggplot(shootings_per_race, aes(y=shootings, x=reorder(race, -shootings))) + 
  ...
  labs(x = "race",
       y = "number of mass shootings") +
  geom_text(aes(label=shootings), vjust = -0.5, size = 8, col = "#666666") +
  theme(
...
lollipop_chart_loc_type <- ggdotchart(shootings_per_loc_type, x = "location.1", y = "shootings",
           #color = "???",                                # Color by groups
           ...
           label = round(shootings_per_loc_type$shootings),                # Add shootings values as dot labels
...
There is a sharp increase in the number of mass shooters in their twenties.
