...
source("Package_Helper.R")

packages <- list("tidyverse", "operators", "knitr", "sf", "usmap", "waffle", "treemapify", "gganimate", "tweenr", "gifski", "png", "viridis", "rlang", "lubridate")

package_loader(packages)

# Tidy-up!
rm(packages, package_loader)
```
...
```{r}
refactorer <- function(df, col_name, level_vec, label_vec, other){
  level_vec <- tolower(level_vec)
  column_sym <- rlang::sym(col_name)
  # note the special syntax i.e. !!column_sum := ...
  # https://www.tidyverse.org/articles/2018/03/rlang-0.2.0/
  df %>% 
    mutate(!!column_sym := trimws(!!column_sym)) %>% #get rid of whitespace...
    mutate(!!column_sym := tolower(!!column_sym)) %>% # change to lower case for better comparisons..
    mutate(!!column_sym := factor(!!column_sym, levels = level_vec, labels = label_vec)) %>%
    mutate(!!column_sym := fct_explicit_na(!!column_sym, na_level =  other)) # use forcats to deal with unset levels...
}
```
