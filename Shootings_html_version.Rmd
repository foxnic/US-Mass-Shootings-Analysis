...
```{r display_plot_state_victims, echo=FALSE, fig.width=10, fig.height=7}
...
animated_plot <- ggplot(data = shootings_per_year_state, aes(x = state, y = shootings, fill = state)) +
  ...
  theme(legend.position = "none",
        axis.ticks = element_blank(),
        axis.text = element_text(colour = "#666666", size = 16, angle = 90, vjust = 0.5),
  ...
  labs(title = "Year: {frame_time}", x = "", y = "Shootings") +
...
```{r create_timeline_summary, include=FALSE}

# Create a new data frame containing only the shootings in 2016-2019
cleansed_data_timeline <- data.frame(cleansed_data[cleansed_data$year > "2015" ,c("case", "international_date", "total_victims")])

# Create a new data frame that shows number of victims per date 
victims_per_date <- aggregate(cleansed_data_timeline$total_victims, by=list(cleansed_data_timeline$international_date),FUN = sum)

# Change the column heading names to "date" and "victims"
colnames(victims_per_date) <- c("date", "victims")

# Sort the data by date
victims_per_date <- victims_per_date[order(as.Date(victims_per_date$date, format="%Y/%m/%d")),]

# Create a new data frame containing the rows in victims_per_date plus rows with 0 victims for all the dates on which there were no mass shootings. Start by creating a list of all the dates between the 1st shooting and the latest.
earliest_shooting <- as.Date(min(victims_per_date$date))
most_recent_shooting <- as.Date(max(victims_per_date$date))
date_range <- as.data.frame(seq(earliest_shooting, most_recent_shooting, by=1))

# Change the column heading name to "date"
colnames(date_range) <- "date"

# Add a victims column with all values set to zero
date_range$victims <- 0

victims_per_date <- right_join(victims_per_date, date_range, by = c("date"="date"))

# Change the column heading names to date, victims, temp_victims
colnames(victims_per_date) <- c("date", "victims", "temp_victims")

# If victims is NA then set it to zero
victims_per_date[is.na(victims_per_date$victims), c("victims")] <- 0

# Remove the temp_victims column
victims_per_date <- victims_per_date[, c("date", "victims")]

# Change zeros to NA so a point isn't displayed on the plot for dates with no shootings
victims_per_date[victims_per_date$victims == 0, "victims"] <- NA

```

```{r display_timeline, echo=FALSE, fig.height=3, fig.width=10, message=FALSE, warning=FALSE}

# Create a single time line with bubbles representing victims by date for shootings between 2016 and 2019

timeline_chart <- ggplot(data = victims_per_date, aes(x = date, y = 0)) +
  geom_hline(yintercept = 0, linetype = "solid", color = "gray", size = 0.75)+
  geom_point(aes(size = victims),
             shape = 21,
             colour = "#dd5202",
             alpha = 0.7,
             fill = "#e27f46")+
  ...
  geom_label(aes(x = as.Date("2018-03-01"), y = 0.06, label = "Thousand Oaks nightclub shooting\n7th Nov 2018.\n34 victims."), 
             hjust = 0, vjust = 0.5, lineheight = 0.8,
             colour = "#FFFFFF", fill = "#dd5202", label.size = NA, family="Helvetica", size = 4)+ 
  geom_segment(aes(x = as.Date("2018-11-07"), y = 0.007, xend = as.Date("2018-10-15"), yend = 0.0375), 
               colour = "#dd5202", size=0.5)+
  geom_label(aes(x = as.Date("2017-08-10"), y = 0.06, label = "Texas First Baptist\nChurch massacre\n5th Nov 2017.\n46 victims."), 
             hjust = 0, vjust = 0.5, lineheight = 0.8,
             colour = "#FFFFFF", fill = "#dd5202", label.size = NA, family="Helvetica", size = 4)+ 
  geom_segment(aes(x = as.Date("2017-11-05"), y = 0.007, xend = as.Date("2017-11-30"), yend = 0.0375), 
               colour = "#dd5202", size=0.5)+
  geom_label(aes(x = as.Date("2016-11-01"), y = 0.06, label = "Las Vegas Strip massacre\n1st Oct 2017.\n604 victims."), 
             hjust = 0, vjust = 0.5, lineheight = 0.8,
             colour = "#FFFFFF", fill = "#dd5202", label.size = NA, family="Helvetica", size = 4)+ 
  geom_segment(aes(x = as.Date("2017-10-01"), y = 0.007, xend = as.Date("2017-07-15"), yend = 0.0375), 
               colour = "#dd5202", size=0.5)+
  geom_label(aes(x = as.Date("2016-01-10"), y = 0.06, label = "Orlando nightclub massacre\n12th June 2016.\n102 victims."), 
             hjust = 0, vjust = 0.5, lineheight = 0.8,
             colour = "#FFFFFF", fill = "#dd5202", label.size = NA, family="Helvetica", size = 4)+ 
  geom_segment(aes(x = as.Date("2016-06-12"), y = 0.007, xend = as.Date("2016-03-30"), yend = 0.0375), 
               colour = "#dd5202", size=0.5)+
    geom_label(aes(x = as.Date("2016-06-06"), y = -0.05, label = "2016"), 
             hjust = 0, vjust = -0.5, lineheight = 0.8,
             colour = "#666666", fill = "white", label.size = NA, family="Helvetica", size = 5)+ 
  geom_segment(aes(x = as.Date("2016-01-01"), y = 0.03, xend = as.Date("2016-01-01"), yend = -0.03), 
               colour = "gray", size=0.5)+
    geom_label(aes(x = as.Date("2017-06-06"), y = -0.05, label = "2017"), 
             hjust = 0, vjust = -0.5, lineheight = 0.8,
             colour = "#666666", fill = "white", label.size = NA, family="Helvetica", size = 5)+ 
  geom_segment(aes(x = as.Date("2017-01-01"), y = 0.03, xend = as.Date("2017-01-01"), yend = -0.03), 
               colour = "gray", size=0.5)+
    geom_label(aes(x = as.Date("2018-06-06"), y = -0.05, label = "2018"), 
             hjust = 0, vjust = -0.5, lineheight = 0.8,
             colour = "#666666", fill = "white", label.size = NA, family="Helvetica", size = 5)+ 
  geom_segment(aes(x = as.Date("2018-01-01"), y = 0.03, xend = as.Date("2018-01-01"), yend = -0.03), 
               colour = "gray", size=0.5)+
    geom_label(aes(x = as.Date("2019-01-30"), y = -0.05, label = "2019"), 
             hjust = 0, vjust = -0.5, lineheight = 0.8,
             colour = "#666666", fill = "white", label.size = NA, family="Helvetica", size = 5)+ 
  geom_segment(aes(x = as.Date("2019-01-01"), y = 0.03, xend = as.Date("2019-01-01"), yend = -0.03), 
               colour = "gray", size=0.5)
...
heatmap_decade_state <- ggplot(shootings_per_decade_state, aes(decade, state)) + 
  ...
  theme(
    axis.text.y = element_text(colour = "#666666", size = 14),
 ...
 plot_state_semi <- ggplot() + 
  geom_bar(aes(y=Num_Shootings, x=State, fill=Semi_Auto_Weapons_Used), 
   ...
   theme(
    ...
    axis.title = element_text(colour = "#666666", size = 14),
    axis.ticks = element_blank(),
    panel.background = element_blank(),
    axis.line.x = element_line(colour = "#666666"),
    plot.margin=unit(c(1,0,1,0),"cm"))+
...
# Create a bar chart using ggplot2 to show shootings by state and race
 plot_state_race <- ggplot() + 
  geom_bar(aes(y=Num_Shootings, x=State, fill=Race), data = shootings_per_state_race, stat = "identity", position= "fill") +
  scale_fill_manual(values = c(colour_palette[50], colour_palette[60], colour_palette[70], colour_palette[80], colour_palette[90])) +
  labs(x = "State",
       y = "% of mass shootings") +
  theme(
...
# Create a bar chart using ggplot2 to show shootings by state and whether weapons were obtained legally
 plot_state_legally <- ggplot() + 
  geom_bar(aes(y=Num_Shootings, x=State, fill=Weapons_Obtained_Legally), data = shootings_per_state_legally, stat = "identity", position= "fill") +
...
# Create a bar chart using ggplot2 to show shootings by state and location type
 plot_state_loc_type <- ggplot() + 
  geom_bar(aes(y=Num_Shootings, x=State, fill=Location_Type), data = shootings_per_state_loc_type, stat = "identity", position = "fill") +
  scale_fill_manual(values = c(colour_palette[40],colour_palette[50], colour_palette[60], colour_palette[70], colour_palette[80], colour_palette[90])) +
  labs(x = "State",
       y = "% of mass shootings",
       fill = "Location Type") +
  theme(
...
# Create a bar chart using ggplot2 to show shootings by state and age bracket
 plot_state_age <- ggplot() + 
  geom_bar(aes(y=Num_Shootings, x=State, fill=Age_Bracket), data = shootings_per_state_age, stat = "identity", position = "fill") +
  scale_fill_manual(values = c(colour_palette[20],colour_palette[40], colour_palette[55], colour_palette[65], colour_palette[75], colour_palette[90], colour_palette[100])) +
  labs(x = "State",
       y = "% of mass shootings",
       fill = "Age Bracket") +
  theme(
...
# Create a bar chart using ggplot2 to show shootings by year and whether semi-automatic weapons were used
 plot_year_semi <- ggplot(aes(y=Num_Shootings, x=Year, colour=Semi_Auto_Weapons_Used), data = shootings_per_year_semi) + 
  geom_line(size = 2) +
  scale_colour_manual(values = c(colour_palette[65], colour_palette[90])) +
  labs(x = "Year",
       y = "Number of mass shootings",
       colour = "Semi-Auto Weapons Used?") +
  theme(
    axis.text.y = element_text(colour = "#666666", size = 10),
    axis.text.x = element_text(colour = "#666666", size = 10),
    axis.ticks = element_blank(),
    panel.background = element_blank(),
    axis.line.x = element_line(colour = "#666666"),
    plot.margin=unit(c(1,0,1,0),"cm"))
...
# Create a bar chart using ggplot2 to show shootings by year and whether multiple weapons were used
 plot_year_multi <- ggplot(aes(y=Num_Shootings, x=Year, colour=Multiple_Weapons_Used), data = shootings_per_year_multi) + 
  geom_line(size = 2) +
  scale_colour_manual(values = c(colour_palette[30], colour_palette[50])) +
  labs(x = "Year",
       y = "Number of mass shootings",
...
# Create a bar chart using ggplot2 to show shootings by year and whether there were prior signs of mental health issues
 plot_year_prior <- ggplot(aes(y=Num_Shootings, x=Year, colour=Prior_Signs_Of_Mental_Health_Issues), data = shootings_per_year_prior) + 
  geom_line(size = 2) +
  scale_colour_manual(values = c(colour_palette[60], colour_palette[80], colour_palette[97])) +
  labs(x = "Year",
       y = "Number of mass shootings",
...
# Create a bar chart using ggplot2 to show shootings by year and whether weapons were obtained legally
 plot_year_legally <- ggplot(aes(y=Num_Shootings, x=Year, colour=Weapons_Obtained_Legally), data = shootings_per_year_legally) + 
  geom_line(size = 2) +
  scale_colour_manual(values = c(colour_palette[25], colour_palette[55], colour_palette[75])) +
  labs(x = "Year",
       y = "Number of mass shootings",
...
# Create a bar chart using ggplot2 to show shootings by year and location type
 plot_year_loc_type <- ggplot(aes(y=Num_Shootings, x=Year, colour=Location_Type), data = shootings_per_year_loc_type) + 
  geom_line(size = 2) +
  scale_colour_manual(values = c(colour_palette[7],colour_palette[35], colour_palette[55], colour_palette[75], colour_palette[87], colour_palette[94])) +
  labs(x = "Year",
       y = "Number of mass shootings",
